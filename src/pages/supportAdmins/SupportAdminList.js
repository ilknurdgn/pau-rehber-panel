...
import './SupportAdminList.css';
import DataTable from '../../components/dataTable/DataTable';
import BaseModal from '../../components/baseModal/BaseModal';

// API endpoints for Support Admins
const LIST_API = 'https://paurehber.ilknurdogan.dev/api/admins/support-admins';
...
const ADMIN_ROLES = [
  { value: 'SUPPORT_ADMIN', label: 'Destek Admini' }
];

const roleLabels = {
    SUPPORT_ADMIN: 'Destek Admini'
  };
...
export default function SupportAdminList() {
  ...
  const [formData, setFormData] = useState({
    firstName: '', lastName: '', email: '', password: '', facultyId: 0, adminRole: 'SUPPORT_ADMIN'
  });
  const [updateData, setUpdateData] = useState({
    id: null, firstName: '', lastName: '', email: '', password: '', phoneNumber: '', role: ''
  });

  // Initial load: support-admins + faculties
  useEffect(() => {
    const init = async () => {
      try {
        const token = localStorage.getItem('token');
        // fetch support-admins
        const res = await fetch(LIST_API, { headers: { Authorization: `Bearer ${token}` } });
        const json = await res.json();
        if (!res.ok || !json.success) throw new Error(json.message);
        setAdmins(json.payload);
        // fetch faculties
        const fRes = await fetch(FACULTIES_API);
        const fJson = await fRes.json();
        if (fJson.success) setFaculties(fJson.payload);
      } catch (e) {
        setError(e.message);
      } finally {
        setLoading(false);
  ...
  const handleCreate = async e => {
    ...
    try {
      ...
      setLoading(true);
      const r2 = await fetch(LIST_API, { headers: { Authorization: `Bearer ${token}` } });
      const j2 = await r2.json();
      setAdmins(j2.payload);
    } catch (e) {
      alert(e.message);
    } finally {
      setLoading(false);
  ...
  const handleUpdate = async e => {
    ...
    try {
      const token = localStorage.getItem('token');
      const payload = {};
      ['firstName','lastName','email','password','phoneNumber','role'].forEach(k => {
        const v = updateData[k];
        if (v != null && v !== '') payload[k] = v;
      });
      const res = await fetch(`${UPDATE_API_BASE}${updateData.id}`, {
        method: 'PATCH',
        headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
  ...
  const handleDelete = async id => {
    if (!window.confirm('Bu support admin silinsin mi?')) return;
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`${DELETE_API_BASE}${id}`, { method: 'DELETE', headers: { Authorization: `Bearer ${token}` } });
  ...
  return (
    <div className="superadmin-page">
      <div className="superadmin-header">
        <h2>Destek Adminler</h2>
        <button className="create-admin-btn" onClick={() => setIsCreateOpen(true)}>Yeni Destek Admin</button>
      </div>
      ...
      {isCreateOpen && (
        <BaseModal title="Yeni Destek Admin Oluştur" isOpen onClose={() => setIsCreateOpen(false)}>
      ...
      {isUpdateOpen && (
        <BaseModal title="Destek Admin Güncelle" isOpen onClose={() => setIsUpdateOpen(false)}>
